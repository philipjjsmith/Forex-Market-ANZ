...
--
-- A signal that trips a risk control is NOT dropped. It is recorded like any other signal --
-- so the forward out-of-sample dataset stays complete -- but flagged as needing a human
-- decision, and never auto-executed. Dropping it silently would both hide the control and
-- put a hole in the only clean dataset this project has left.
--
-- requires_approval  true  = generated, delivered, but held back from automatic execution
-- approval_reason          = why, in words, e.g. the measured correlation exposure
--
-- The deduped view enumerates every column explicitly, so it must be recreated to carry these.
-- The column list below is the 2026-08-30 definition verbatim plus the two new columns; the
-- DISTINCT ON expression and ORDER BY are UNCHANGED, so deduplication behaviour is identical.
...
COMMENT ON COLUMN signal_history.requires_approval IS
  'True when a risk control held this signal back from automatic execution. The signal is still '
  'a real, delivered signal and MUST remain in every statistic; it simply was not auto-traded.';
...
COMMENT ON VIEW signal_history_deduped IS
  ...
  'requires_approval/approval_reason (2026-08-31) mark signals a risk control held back from '
  'automatic execution - they are still real signals and belong in every statistic.';
