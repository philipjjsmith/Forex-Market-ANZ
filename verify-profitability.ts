...
async function verifyProfitability() {
  ...
  try {
    ...
    const beforeAfterCounts = await db.execute(sql`
      SELECT
        CASE
    ...
    const afterSignals = await db.execute(sql`
      SELECT
        signal_id,
        symbol,
        type,
    ...
    const afterPerformance = await db.execute(sql`
      SELECT
        COUNT(*) as total_signals,
        COUNT(*) FILTER (WHERE trade_live = true) as live_trades,
    ...
    if (completedTrades === 0) {
      ...
      const signalsPerDay = afterSignals.length / daysElapsed;
    ...
    const beforePerformance = await db.execute(sql`
      SELECT
        COUNT(*) as total_signals,
        COUNT(*) FILTER (WHERE trade_live = true) as live_trades,
        COUNT(*) FILTER (WHERE outcome = 'PENDING') as pending,
