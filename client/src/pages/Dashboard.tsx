...
export default function Dashboard() {
  ...
  // Removed quota tracking - signals are now fetched automatically from database
  ...
  // Fetch active signals from database (automated + manual)
  const fetchActiveSignals = useCallback(async () => {
    try {
      if (import.meta.env.DEV) {
        console.log('📡 Fetching active signals from database...');
      }
      ...
      if (data.signals && Array.isArray(data.signals)) {
        // Convert database signals to UI format
        const formattedSignals = data.signals.map((dbSignal: any) => ({
          ...
          stop: dbSignal.stop_loss,
          targets: [dbSignal.tp1, dbSignal.tp2, dbSignal.tp3],
          riskReward: ((dbSignal.tp1 - dbSignal.entry_price) / (dbSignal.entry_price - dbSignal.stop_loss)),
          confidence: dbSignal.confidence,
          tier: dbSignal.tier,
          tradeLive: dbSignal.trade_live,
          positionSizePercent: dbSignal.position_size_percent,
          orderType: dbSignal.order_type,
          executionType: dbSignal.execution_type,
          indicators: dbSignal.indicators || {},
          rationale: `${dbSignal.tier} tier signal generated by automated ICT 3-TF analysis`,
          strategy: 'MA Crossover Multi-Timeframe',
          version: '3.1.0',
          status: 'active'
        }));
        ...
        if (import.meta.env.DEV) {
          console.log(`✅ Loaded ${formattedSignals.length} active signals`);
        }
      }
    } catch (error: any) {
      console.error('❌ Error fetching signals:', error);
      setApiError(error.message || 'Failed to fetch signals');
    }
  }, [toast]);

  // Fetch signals on mount and every 5 minutes
  useEffect(() => {
    fetchActiveSignals();

    const interval = setInterval(fetchActiveSignals, 5 * 60 * 1000); // Poll every 5 minutes
